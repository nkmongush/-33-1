document.addEventListener('DOMContentLoaded', function () {
    ...
    function initRegistration() {
        ...
        // setupValidation(nameInput, dateInput, nameError, dateError); // Removed setupValidation call
    }
    ...
    function updateProfileDisplay() {
        ...
        try {
            ...
            if (registrationData) {
                ...
                // Важная корректировка: проверка существования данных перед использованием
                nameDiv.textContent = data.name ? `Имя: ${data.name}` : '';
                dateDiv.textContent = data.date ? `Дата рождения: ${data.date}` : '';
                genderDiv.textContent = data.gender ? `Пол: ${data.gender}` : '';

                profileSlide.style.display = 'block'; // Отображаем блок профиля, только если есть данные
            } else {
                profileSlide.style.display = 'none'; // Скрываем, если данных нет
            }
        } catch (error) {
            showCustomError("Ошибка загрузки данных профиля.", profileSlide);
            console.error("Error parsing registration data:", error);
            profileSlide.style.display = 'none'; // Скрыть, если ошибка
        }
    }
